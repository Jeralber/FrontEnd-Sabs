...
import MunicipioPDF from './MunicipioPDF';
import { Models } from '../../types/types';

// Tipo de datos a mostrar
type ReportType = "normal" | "activos" | "inactivos" | "recientes";

interface MunicipioReportSelectorProps {
  data: Partial<Models['Municipio']>[];
  startDate?: string;
  endDate?: string;
}

const MunicipioReportSelector: React.FC<MunicipioReportSelectorProps> = ({ data, startDate, endDate }) => {
  ...
  const getReportName = (type: ReportType): string => {
    switch (type) {
      ...
      case "activos": return "Municipios Activos";
      case "inactivos": return "Municipios Inactivos";
      case "recientes": return "Municipios Recientes";
    ...
    <div className="p-4 border rounded-lg shadow-sm bg-white dark:bg-gray-800">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        >
          ...
          <span>Municipios Inactivos</span>
        </button>
        ...
        >
          <FaClock size={14} />
          <span>Municipios Recientes</span>
        </button>
      </div>
      
      <PDFDownloadLink 
        document={<MunicipioPDF data={data} startDate={startDate} endDate={endDate} reportType={reportType} />} 
        fileName={`reporte-municipios-${reportType}-${new Date().toISOString().split('T')[0]}.pdf`}
...
export default MunicipioReportSelector;