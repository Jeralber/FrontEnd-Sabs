...
import { FaFilePdf, FaCheckCircle, FaTimesCircle, FaExchangeAlt, FaClock } from "react-icons/fa";
import MovimientosPDF from './MovimientosPDF';
import { Models } from '../../types/types';

// Tipo de datos a mostrar
type ReportType = "normal" | "activos" | "inactivos" | "porTipo" | "recientes";

interface MovimientoReportSelectorProps {
  data: Partial<Models['Movimiento']>[];
  startDate?: string;
  endDate?: string;
}

const MovimientoReportSelector: React.FC<MovimientoReportSelectorProps> = ({ data, startDate, endDate }) => {
  ...
  const getReportName = (type: ReportType): string => {
    switch (type) {
      ...
      case "activos": return "Movimientos Activos";
      case "inactivos": return "Movimientos Inactivos";
      case "porTipo": return "Por Tipo de Movimiento";
      case "recientes": return "Movimientos Recientes";
    ...
    <div className="p-4 border rounded-lg shadow-sm bg-white dark:bg-gray-800">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        >
          ...
          <span>Movimientos Inactivos</span>
        </button>
        ...
        >
          ...
          <span>Por Tipo de Movimiento</span>
        </button>
      ...
      <PDFDownloadLink 
        document={<MovimientosPDF data={data} startDate={startDate} endDate={endDate} reportType={reportType} />} 
        fileName={`reporte-movimientos-${reportType}-${new Date().toISOString().split('T')[0]}.pdf`}
...
export default MovimientoReportSelector;