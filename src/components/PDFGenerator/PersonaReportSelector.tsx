...
import { FaFilePdf, FaCheckCircle, FaTimesCircle, FaUsers, FaClock } from "react-icons/fa";
import PersonasPDF from './PersonasPDF';
import { Models } from '../../types/types';

// Tipo de datos a mostrar
type ReportType = "normal" | "activas" | "inactivas" | "porRol" | "recientes";

interface PersonaReportSelectorProps {
  data: Partial<Models['Persona']>[];
  startDate?: string;
  endDate?: string;
}

const PersonaReportSelector: React.FC<PersonaReportSelectorProps> = ({ data, startDate, endDate }) => {
  ...
  const getReportName = (type: ReportType): string => {
    switch (type) {
      ...
      case "activas": return "Personas Activas";
      case "inactivas": return "Personas Inactivas";
      case "porRol": return "Por Rol";
      case "recientes": return "Personas Recientes";
  ...
  return (
    <div className="p-4 border rounded-lg shadow-sm bg-white dark:bg-gray-800">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        >
          <FaClock size={14} />
          <span>Personas Recientes</span>
        </button>
      </div>
      
      <PDFDownloadLink 
        document={<PersonasPDF data={data} startDate={startDate} endDate={endDate} reportType={reportType} />} 
        fileName={`reporte-personas-${reportType}-${new Date().toISOString().split('T')[0]}.pdf`}
...
export default PersonaReportSelector;